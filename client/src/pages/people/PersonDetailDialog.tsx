...
export const PersonDetailDialog = ({ userId, onClose }: PersonDetailDialogProps) => {
  const queryClient = useQueryClient();
  const { user: currentUser } = useAuth();
  const canEditUser = hasPermission(currentUser?.permissions, Permission.USER_EDIT);
  ...
  const employee = employeesData?.employees.find((e) => e.user._id === userId);
  ...
  const onLinkSubmit = async (values: LinkEmployeeValues) => {
    try {
      await employeesApi.linkEmployee({ ...values, user: userId });
      await invalidateAll();
  ...
  const loading = userLoading || employeesLoading;

  return (
    <Modal open onClose={onClose} title={fullName || 'Person details'} size="lg">
      {loading && <PageSpinner />}
      {!loading && !user && <p className="text-sm text-slate-500">Person not found.</p>}
      {!loading && user && (
          ...
          <Card>
            ...
            <CardBody>
              ...
              >
                ...
                <RoleGate permission={Permission.USER_EDIT}>
              ...
              <Card>
                ...
                <CardBody>
                  <p className="mb-4 text-sm text-slate-500">
                    This account isn&apos;t linked to an employee record yet. Fill this in to
                    track {fullName}&apos;s department, designation, and employment status.
                  </p>
                  <form
